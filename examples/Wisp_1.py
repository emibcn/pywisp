...
class MyWISP():
    ...
    def get_ac_devices(devices=None):
        ...
        for dev in devices:
            ...
            antenas.append(ACDevice(dev, username='admin',
    ...
    def get_mt_devices(self):
        ...
        # Get VACCs from CCR
        antenas, ccr = self.get_dhcp_clients_from_ccr_ip(
            "10.255.255.7", password_ccr=PASSWORD, password_devices=PASSWORD_CLIENT, find="^VACC.*")
        print("CCR: {ccr} (VACCs): {count} VACCs".format(
            ccr=str(ccr), count=str(len(antenas))))

        # Get RTs from all CCRs, ans save CCRs list
        ccrs = []
        for ccr_ip in self.get_ccrs_ips():
            antenas_ccr, ccr = self.get_dhcp_clients_from_ccr_ip(
                ccr_ip, password_ccr=PASSWORD, password_devices=PASSWORD, find=".*-RT")
            print("CCR: {ccr}: {count} RTs".format(
                ccr=str(ccr), count=len(antenas_ccr)))
    ...
    def get_host(self, name, deep=False, from_br=None):
        ...
        if name.startswith('vacc'):
            devices, ccr = self.get_dhcp_clients_from_ccr_ip(
                "10.255.255.7", password_ccr=PASSWORD, password_devices=PASSWORD_CLIENT, find="^{}.*".format(name.upper()))
    ...
    def get_dhcp_clients_from_ccr(ccr, password_ccr="", password_devices="", find=""):
        ...
        # Generate antennas list (with credentials)
        devices = [
            MTDevice(client, username='admin',
                     password=password_devices, rsa=ID_RSA)
            for client in clients
        ]
    ...
    def get_ccrs(self):
        return [
            MTDevice({}, username='admin',
                     password=PASSWORD, rsa=ID_RSA, ip=ip)
    ...
    def get_rts(self, name, ccrs):
        ...
        devices = []
        for ccr in ccrs:
            devices_ccr = self.get_dhcp_clients_from_ccr(
                ccr, password_ccr=PASSWORD, password_devices=PASSWORD, find=".*{}-RT".format(name[:-3]))
